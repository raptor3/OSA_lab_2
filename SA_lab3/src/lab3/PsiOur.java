...
public class PsiOur {
    ...
    public PsiOur(int n1, int n2, int n3, int p1, int p2, int p3,
            Polinom pol, Matr lambda,Matr v, double[][] maxx, double[][] minx) {
                ...
                for (int pi = 0; pi <= p[s]; pi++) {
                    for (int p4 = 0; p4 <= p[s]; p4++) {
                        try {
                            ...
                            Logger.getLogger(PsiOur.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                }
            }
    ...
    public double psi_s_js(int s, int js, double x) {
        ...
        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                //sum += lambda.getX(s)[js][i] * Math.log(1+Func.func(pol.func(i, x)));
                sum *= Math.pow(1+Func.func(pol.func(i, x)), v.getX(s)[js][i]);
                if (Double.isInfinite(sum)) {
                    System.out.println(lambda.getX(s)[js][i]);
                    System.out.println(Math.log(1+pol.func(i, x)));
                    System.out.println(pol.func(i, x));
                    System.out.println("");
                }
            } catch (Exception ex) {
                Logger.getLogger(PsiOur.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if (Double.isInfinite(Math.exp(sum))) {
     ...
     public double ln_psi_s_js_1(int s, int js, double x) {
        double sum = 0;

        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                sum += v.getX(s)[js][i] * Math.log(Math.abs(1+Func.func(pol.func(i, x)))+10e-6);
                if (Double.isNaN(sum)) {
                   // System.out.println(lambda.getX(s)[js][i]);
                    //System.out.println(Math.log(1+pol.func(i, x)));
                    System.out.println(pol.func(i, x));
    ...
    public String toString(double number, int s, int js) {
        ...
        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                str.append("+" + lambda.getX(s)[js][i] + "*ln(1+" + "arcsin(T" + i + "(x" + s + js + ")))");
            } catch (Exception ex) {
                Logger.getLogger(PsiOur.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
