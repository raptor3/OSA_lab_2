...
public class Psi {
    ...
    public double psi_s_js(int s, int js, double x) {
        ...
        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                sum += lambda.getX(s)[js][i] * Math.log(1+pol.func(i, x));
     ...
     public double ln_psi_s_js_1(int s, int js, double x) {
        ...
        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                sum += v.getX(s)[js][i] * Math.log(Math.abs(1+lambda.getX(s)[js][i] * pol.func(i, x))+10e-6);
                if (Double.isNaN(sum)) {
                    System.out.println(lambda.getX(s)[js][i]);
                    //System.out.println(Math.log(1+pol.func(i, x)));
                    System.out.println(pol.func(i, x));
    ...
    public String toString(double number, int s, int js) {
        ...
        for (int i = 0; i <= p[s - 1]; i++) {
            try {
                str.append("+" + lambda.getX(s)[js][i] + "*ln(1+" + "T" + i + "(x" + s + js + "))");
            } catch (Exception ex) {
                Logger.getLogger(Psi.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
